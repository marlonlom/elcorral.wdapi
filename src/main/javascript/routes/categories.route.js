...
const jsonPath = path.join(
  ...
  "main",
  "json",
  "food_categories.data.json"
);
const foodGroupsPath = path.join(
  process.cwd(),
  "src",
  "main",
  "json",
  "foods_grouped_by_category.data.json"
);
...
router.get("/", (req, res) => {
  fs.readFile(jsonPath, "utf-8", (err, data) => {
    ...
    res.status(200).json({
      results: JSON.parse(data).map((row, pos) =>
        Object.assign(
          {
            id: pos + 1,
          },
...
router.get("/:category_id", (req, res) => {
  fs.readFile(jsonPath, "utf-8", (err, data) => {
    ...
    const singleCategory = JSON.parse(data)
      .map((row, pos) =>
        Object.assign(
          {
            id: pos + 1,
      ...
      .find((itm) => itm.id === Number(req.params.category_id));
    if (!singleCategory) {
      res.status(500).json({
